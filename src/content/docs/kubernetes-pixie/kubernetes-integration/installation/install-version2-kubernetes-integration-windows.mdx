...
For a useful mapping between release IDs and OS versions, see [here](the Docker Windows Server Core docs).
...
4. Create a `values.yml` file with the follow data to be used by Helm:
  ...
  nodeSelector:
    kubernetes.io/os: linux # Node selector for Linux
  windowsNodeSelector:
    kubernetes.io/os: windows # Node selector for Windows
  ```
