---
title: Install Kubernetes integration in Windows
tags:
  - Integrations
...
3. Install kube-state-metrics.
  ...
  helm install ksm  prometheus-community/kube-state-metrics --version 5.2.0  
  ```

<Callout variant="important">
  The command above allows installing kube-state-metrics, which is a mandatory dependency of the integration, on a linux node. By any chance if kube-state-metrics are scheduled on a non-linux node, it might fail to create pods successfully. Generally as k8s control plane designed to run on a linux node, cluster will have a linux node. So, use nodeSelector and get kube-state-metrics to deploy on a linux node. This will get the kube-state-metrics up. If your cluster does not have any linux node, the command above might not work. New Relic cannot offer support on how to install kube-state-metrics on non-linux nodes.
</Callout>
